...
<div class="form-row">
    ...
    <div class=" hidden form-group col-xs-11{{ $errors->has('sender_id') ? ' has-error' : '' }} mb-0 mt-3">
        ...
        <select class="form-control" id="sender_id" name="sender_id">
            ...
            {{-- @foreach($users as $user)
                <option value="{{$user->id}}" @if (old('sender_id') == $user->id) {{ 'selected' }} @endif>{{$user->name}}</option>
            @endforeach --}}
        </select>
        ...
        <select class="form-control" id="unit_from_id" name="unit_from_id">
            ...
            {{-- @foreach($units as $unit)
                <option value="{{$unit->id}}" @if (old('unit_from_id') == $unit->id) {{ 'selected' }} @endif>{{$unit->name}}</option>
            @endforeach --}}
        </select>
    ...
    <div class="form-group col-xs-11{{ $errors->has('collector_id') ? ' has-error' : '' }} mb-0 mt-3">
        ...
        <select class="form-control" id="collector_id" name="collector_id">
            <option value=""></option>
            @foreach($users as $user)
                <option value="{{$user->id}}" @if (old('collector_id') == $user->id) {{ 'selected' }} @endif>{{$user->name.'--'. \App\Department::find($user->id)->name.'--'.$user->unit->name}}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-xs-11{{ $errors->has('reciever_id') ? ' has-error' : '' }} mb-0 mt-3">
        ...
        <select class="form-control" id="reciever_id" name="reciever_id">
            <option value=""></option>
            @foreach($users as $user)
                <option value="{{$user->id}}" @if (old('reciever_id') == $user->id) {{ 'selected' }} @endif>{{$user->name.'--'. \App\Department::find($user->id)->name.'--'.$user->unit->name}}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-xs-11{{ $errors->has('unit_to_id') ? ' has-error' : '' }} mb-0 mt-3">
        <label for="unit_to_id">Destination Unit:  <small class="text-muted">(reciever destination unit)</small></label>
        <select class="form-control" id="unit_to_id" name="unit_to_id">
            <option value=""></option>
            @foreach($units as $unit)
                <option value="{{$unit->id}}" @if (old('unit_to_id') == $unit->id) {{ 'selected' }} @endif>{{$unit->name.'--'.$unit->department->name}}</option>
            @endforeach
        </select>
